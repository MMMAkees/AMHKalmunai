...
import { departmentTraffic } from '../../data/analytics';
    ...
    <div>
      ...
      <div className="grid sm:grid-cols-3 gap-4 mb-8">
        <div className="bg-white rounded-xl border border-gray-100 p-5 text-center">
          ...
          <p className="text-3xl font-bold text-primary-700">{queues.length}</p>
        </div>
        <div className="bg-white rounded-xl border border-gray-100 p-5 text-center">
          <p className="text-sm text-gray-500">Total Waiting</p>
          <p className="text-3xl font-bold text-orange-600">{queues.reduce((s, q) => s + q.totalWaiting, 0)}</p>
        </div>
        <div className="bg-white rounded-xl border border-gray-100 p-5 text-center">
          <p className="text-sm text-gray-500">Avg Wait Time</p>
          <p className="text-3xl font-bold text-teal-600">{Math.round(queues.reduce((s, q) => s + q.avgWaitTime, 0) / queues.length)}m</p>
        </div>
      </div>

      <ChartCard title="Department Performance" subtitle="Patient volume by department" className="mb-8">
